...
LAST EDITED:  20.06.2022
    - class CLIP includes several updates required for downstream tasks
ABOUT SCRIPT:
It defines the CLIP model architecture and its layers.
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # DEFAULT
        self.external_image_arch_header = None
        self.external_dim_reduction = None
        ...
        self.task_type=downstream_task
    ...
    def encode_image(self, image, label=None):
        ...
        if self.external_dim_reduction is not None:
            image=self.external_dim_reduction(image)

        image_features=self.visual(image)
        image_features = image_features / image_features.norm(dim=1, keepdim=True)

        if self.external_image_arch_header is not None:
            image_features = self.external_image_arch_header(image_features, label)

        return image_features

    def project_image(self, feature):
        if self.external_image_mlp_header is not None:
            feature= self.external_image_mlp_header(feature)
        return feature



    def encode_text(self, text,label=None):
        ...
        if self.external_text_arch_header is not None:
            x = self.external_text_arch_header(x, label)

        return x

    def forward(self, image, text=None, label=None):
        image_features = self.encode_image(image,label)
        image_features = self.project_image(image_features)

        if self.external_image_mlp_header is not None:
            return image_features

        text_features = self.encode_text(text,label)
